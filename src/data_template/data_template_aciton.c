...
static void _on_action_handle_callback(char *msg, void *context) 
{
	...
	Log_d("Receive action message: %s", msg);

	// prase_method
    if (!parse_template_method_type(msg, &type_str))
    {
	...
	// prase action ID
	if (!parse_action_id(msg, &action_id)) {
        Log_e("fail to parse action id!");
        goto EXIT;
    }

	// prase timestamp
	if (!parse_timestamp(msg, &timestamp)) {
        Log_e("fail to parse timestamp!");
        goto EXIT;
    }

	// prase action input
	if (!parse_action_input(msg, &pInput)) {
...
static int _iot_action_json_init(void *handle, char *jsonBuffer, size_t sizeOfBuffer, const char *pClientToken, DeviceAction *pAction) 
{
	...
	rc_of_snprintf = HAL_Snprintf(jsonBuffer, sizeOfBuffer, "{\\\"method\\\":\\\"%s\\\""T_",\\\"clientToken\\\":\\\"%s\\\""T_",", REPORT_ACTION, pClientToken);
#else
...
#ifdef QUOTES_TRANSFER_NEED
	rc_of_snprintf = HAL_Snprintf(jsonBuffer + strlen(jsonBuffer), remain_size, "\\\"code\\\":%d"T_",\\\"status\\\":\\\"%s\\\""T_",\\\"response\\\":{",\
...
int IOT_Action_Init(void *c)
{
	...
	rc = module_mqtt_sub(topic_name, QOS0, _on_action_handle_callback, c);
	if (rc != QCLOUD_RET_SUCCESS) 
	{
...
int IOT_Action_Remove(void *pTemplate, DeviceAction *pAction)
{
	Qcloud_IoT_Template *ptemplate = (Qcloud_IoT_Template *)pTemplate;
	if (!_check_action_existence(ptemplate, pAction)) 
		IOT_FUNC_EXIT_RC(QCLOUD_ERR_ACTION_NOT_EXIST);
